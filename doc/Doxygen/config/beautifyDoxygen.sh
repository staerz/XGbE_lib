...
# first source the LASP functions
#LASPSETUP="$(readlink -f ${BASH_SOURCE[0]} | xargs -i dirname {} | sed 's/scripts.*/scripts/')/laspFunctions.sh"
#if [ ! -f ${LASPSETUP} ]; then
#  echo "FATAL: LASP functions script '${LASPSETUP}' not found!"
#  exit 2
#else
#  source ${LASPSETUP} &> /dev/null
#fi
